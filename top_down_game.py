...
class Game:
    ...
    def _spawn_next_wave(self) -> None:
        ...
        # Calculate delay until next wave (use spawn_delay from config if available, or default to 3 seconds)
        if self.current_wave < len(waves):
            next_wave_info = waves[self.current_wave]
            delay_to_next = next_wave_info[2] - spawn_delay if len(next_wave_info) > 2 else 3000
            self.wave_timer = max(0, delay_to_next)
        else:
            # Last wave spawned, set timer to check for level completion
    ...
    def _spawn_next_wave_autogenerated(self) -> None:
        ...
        num_waves = min(3 + (self.game_level // 10), 8)  # 3-8 waves
        num_waves_per_level = max(3, num_waves - self.current_wave)
        ...
        # Determine enemy types and count based on level
        total_enemies = 5 + (self.game_level // 2)  # More enemies as level increases
        pentagon_ratio = min(0.4, 0.02 * self.game_level)
        triangle_ratio = min(0.6, 0.3 + (0.015 * (self.game_level - 20)))
        ...
        self.current_wave += 1
        self.wave_timer = 3000  # 3 second delay between auto-generated waves
    ...
    def update_game_level_progression(self) -> None:
        ...
        # Check if all enemies defeated and ready for next wave
